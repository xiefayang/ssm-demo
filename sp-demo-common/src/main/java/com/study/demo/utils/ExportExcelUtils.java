...
public class ExportExcelUtils {
    ...
    public void export() throws Exception{
        try{
            ...
            XSSFRow rowRowName = sheet.createRow(0);                // 在索引0的位置创建行(最顶端的行开始的第一行)
...
//                    response.setHeader("Content-Disposition", headStr);
     ...
     * 批量导出数据
     * */
    public void exportBatchExcel(Map<String, List<Object[]>> map) throws Exception{
        OutputStream out = response.getOutputStream();
        ...
        for (String key : map.keySet()) {
            try{
                ...
                // 定义所需列数
                int columnNum = rowName.length;
                Row rowRowName = sheet.createRow(0);                // 在索引0的位置创建行(最顶端的行开始的第一行)
                ...
                for(int i=0;i<map.get(key).size();i++){
                    ...
                    Object[] obj = map.get(key).get(i);//遍历每个对象
                ...
                if(workbook != null){
                    try
                    {
                        ZipEntry entry = new ZipEntry(key + ".xls");
    ...
    public CellStyle getColumnTopStyle(Workbook workbook) {
        ...
        //设置底边框颜色;
        //style.setBottomBorderColor(XSSFCellStyle.BLACK.index);
        //设置左边框;
        style.setBorderLeft(XSSFCellStyle.BORDER_THIN);
        //设置左边框颜色;
        //style.setLeftBorderColor(XSSFCellStyle.BLACK.index);
        //设置右边框;
        style.setBorderRight(XSSFCellStyle.BORDER_THIN);
        //设置右边框颜色;
        //style.setRightBorderColor(XSSFCellStyle.BLACK.index);
        //设置顶边框;
        style.setBorderTop(XSSFCellStyle.BORDER_THIN);
        //设置顶边框颜色;
        //style.setTopBorderColor(XSSFCellStyle.BLACK.index);
        //在样式用应用设置的字体;
        style.setFont(font);
        //设置自动换行;
